  ...
  methods: {
    ...
    _initData() {
      getHomeData().then(response => {
        ...
        // 给轮播组件 sowing_list赋值
        this.sowing_list = response.list[0].icon_list
        // navList 赋值
        this.nav_list = response.list[2].icon_list
        // 给限时抢购赋值
        this.flash_sale_product_list = response.list[3].product_list
        // 给Tabbar 商品列表赋值
        this.tabbar_all_product_list = response.list[12].product_list
        // 给特色专区赋值
        this.specialZone = response.special_zone
        // 获取首页广告图
        this.home_ad = response.home_ad.image_url
      })
    }
  }
}
