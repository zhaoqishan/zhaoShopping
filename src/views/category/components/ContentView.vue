...
<template>
  <div class="wrapper">
    <!-- 可滑动的标题 -->
    <div ref="subTitleWrapper" class="subTitleWrapper">
      <ul ref="ulContent">
        <li
    ...
    <!-- 下拉菜单 -->
    <div v-show="isShowDropMenu" class="showMenu" @click="menuClick">
      <span v-if="menuDown" class="downMenu">
        <svg-icon icon-class="down" style="width:1.5rem;height:1.5rem" />
      </span>
      <span v-else class="upMenu">
        <svg-icon icon-class="up" style="width:1.5rem;height:1.5rem" />
      </span>
    </div>
    <!-- 下拉菜单内容 -->
    <DropMenu
      :menu-down="!menuDown"
      :categories-detail-data="categoriesDetailData"
      :current-sub-title="currentSubTitle"
      @itemClick="itemClick"
      @touchClick="menuClick"
    />

    <!-- 商品内容列表 -->
    <section ref="r_list" class="r_list">
      <div ref="b">
        <div v-for="(it,index) in categoriesDetailData" :key="index" ref="good">
          <p class="productCategoryTitle">{{ it.name }}</p>
          <ul>
            <li
              v-for="(item,key) in it.products"
              :key="key"
              class="list"
...
export default {
  ...
  methods: {
    ...
    subTitleClick(index) {
      ...
      const el = this.$refs.subTitle[index]
    ...
    _isShowDropMenu() {
      // 如果标题超出则显示下拉框按钮
      const subTitleWrapperWidth = this.$refs.subTitleWrapper.clientWidth
      const ulContentWidth = this.$refs.ulContent.clientWidth
