  ...
  <div id="mine">
    ...
    <van-cell-group style="margin-top:2.4rem">
      ...
      >
        <template slot="title">
          <!-- 已登录状态 -->
          <div v-if="userInfo.token" class="personMsg" @click="goToPage('userCenter')">
            <img class="iconImage" :src="user_image.login_icon" alt />
            <div v-if="userInfo.sex" class="sex">
              <img :src="userInfo.sex=='1'?user_image.female:user_image.male" alt />
            </div>
            <div v-if="userInfo.token" class="personInfo">
              <!-- <span>{{ userInfo.user_name }}</span> -->
              <span>{{ $t('zhaoqishan') }}</span>
    ...
    <van-cell-group style="margin-top:0.4rem">
      ...
      <!-- 语言切换 -->
      <van-cell :title="$t('mine.switchLanguage')" icon="clock" is-link @click="onSwitchLanguage" />
    </van-cell-group>
