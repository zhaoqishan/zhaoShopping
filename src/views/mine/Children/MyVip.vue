  ...
  <div id="myVip" ref="myVip">
    <div v-if="!isShowLoading">
      ...
      <div class="coupons">
        ...
        <div class="quanBox">
          <div v-for="(item) in todayTicket" :key="item.id" class="quan" @click="getCoupons">
            <div class="money">
              <i>¥</i>
              {{ item.money }}
            </div>
        ...
        <div class="quanBox">
          <div v-for="(item) in weekTicket" :key="item.id" class="quan" @click="getCoupons">
            <div class="money">
              <i>¥</i>
              {{ item.money }}
            </div>
      ...
      <div class="coupons">
        ...
        <van-sticky :offset-top="40">
          <VipMenuTitleScroll :menu-titles-array="cate" @menuTitleClick="menuTitleClick" />
        </van-sticky>
        <!-- Vip商品列表 -->
        <VipGoodsItems :vip-cate-detail="cateDetail" />
      </div>
    </div>
    <!-- 底部按钮 -->
    <transition name="fade">
      <div v-show="isShowBottomBtn" class="bottomJoinVip" transiton="fade">
...
export default {
  ...
  methods: {
    ...
    // 数据请求
    async _initData() {
      await getVipContent().then(response => {
        // 设置数据
        this.todayTicket = response.today_ticket.tickets
        this.weekTicket = response.week_ticket.tickets
        this.cate = response.cate
        this.cateDetail = response.cate_detail
        // 隐藏动画
