...
<template>
  <div id="allMenuWrapper">
    <div v-show="isShowMenuList" ref="menuLists" class="menuLists">
      <div class="menuWrapper">
        <span v-if="recentlyChooseLists.length>0" class="menuCategoryTitle">{{ $t('eat.recent') }}</span>
        <ul class="contentTip">
          <li v-for="(item) in recentlyChooseLists" :key="item.id" class="item">{{ item.name }}</li>
        </ul>
