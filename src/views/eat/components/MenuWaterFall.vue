...
<template>
  <div class="container-water-fall">
    <waterfall :col="col" :data="data" @loadmore="loadmore">
      <div v-for="(item) in data" :key="item.id" class="cell-item">
        <img v-lazy="item.image" />
        <div class="item-body">
          <div class="item-desc">{{ item.name }}</div>
          <div class="item-footer">
            <img v-lazy="item.author_avatar" class="avatar" alt />
...
export default {
  ...
  methods: {
    ...
    async _initData() {
      // 第一条数据
      await getTodayMenuDetail('/lk01', '').then(response => {
        this.menulistDetail = response.big_recommend.list
        this.data = response.big_recommend.list
      })
    },
    loadmore(num) {
      ...
      getTodayMenuDetail(param, '').then(response => {
        if (response.success) {
          const newArray = response.big_recommend.list
