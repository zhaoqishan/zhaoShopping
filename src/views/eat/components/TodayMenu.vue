...
<template>
  <div>
    <div v-if="!isShowLoading" id="todayMenu">
      <div class="todayMenuWrapper">
        ...
        <div class="menuAll" @click="clickAll">
          ...
          <span v-if="menuDown" class="downMenu">
            <svg-icon icon-class="down" style="width:1rem;height:1rem" />
          </span>
          <span v-else class="upMenu">
            <svg-icon icon-class="up" style="width:1rem;height:1rem" />
          </span>
        </div>
      </div>
...
export default {
  ...
  watch: {
    menuDown() {
      const all = this.$t('eat.all')
      const close = this.$t('eat.close')
  ...
  methods: {
    // 1.获取网络数据
    async _initData() {
      await getTodayMenuCategoryList().then(response => {
        // 1.3. 隐藏loading框
        this.isShowLoading = false
        this.todayMenuCategoryLists = response.list
        this.$nextTick(() => {
    ...
    menuItemClick(index) {
      ...
      const el = this.$refs.menuTitle[index]
