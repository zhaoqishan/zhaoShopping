  ...
  <div class="cart">
    ...
    <div class="cartWrapper">
      ...
      <div v-show="!isShowEmptyCart" class="emptyCart">
        ...
        <router-link :to="{name:'home'}" class="goHome">{{ $t('car.walk') }}</router-link>
      </div>
      <!-- 购物车有数据 -->
      <div v-show="isShowEmptyCart" class="contentWrapper">
        <div v-for="(goods) in shopCart" :key="goods.id" class="contentWrapperList">
          <section>
...
export default {
  ...
  methods: {
    ...
    // 2.数据加载
    async _initData() {
      await getGuessYouLike().then(response => {
        this.youLike_product_lists = response.product_list
