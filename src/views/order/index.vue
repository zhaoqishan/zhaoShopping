...
<template>
  <div id="order">
    ...
    <van-cell-group>
      ...
      <TimeIntervalList
        ref="timeInterval"
        :show-date-time-pop-view="showDateTimePopView"
      ...
      <div class="wrapper" @click="goToGoodsList">
        <div ref="productImageWrapper" class="productImageWrapper">
          <ul ref="imageWrapper" class="imageWrapper">
            <li v-for="(item,index) in goods" ref="subWrapper" :key="index" style="display:inline">
    ...
    <van-cell-group style="margin-top: 0.6rem">
      ...
      <van-cell>
        ...
        <van-switch slot="right-icon" v-model="checked" active-color="#07c160" @input="onInput" />
      </van-cell>
    </van-cell-group>
    <!-- 备注 -->
    <van-cell-group style="margin-top: 0.6rem">
    ...
    <van-cell-group style="margin-top: 0.6rem">
      ...
      <van-cell v-show="isShowPreferential" :title="$t('order.point')">
...
export default {
  ...
  computed: {
    ...
    actualPrice() {
      ...
      let finalPrice
      if (this.checked) {
        const discountsPrice = this.integralToprice
          .toFixed(2)
          .toString()
          .replace('.', '')
  ...
  methods: {
    ...
    onSubmit() {
      if (!this.address_name) {
        ...
        // console.info(this.$t(this.deliveryTime))
        Toast({
    ...
    // 4.switch开关
    onInput(checked) {
      const discountsPrice = this.integralToprice * 100
      // 4.1 判断积分使用条件是否满足
